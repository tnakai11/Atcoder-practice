...
// 2L <= B + C <= R を満たす B,C のペア数
// 各値 A について comb(A-2L+1,1) = A-2L+1 ペア
// (A = R までの総計) = R(R+1)/2 + (-2L+1)R ペア 
// (A = 2L-1 までの総計) = 2L(2L-1)/2 + (-2L+1)(2L-1) ペア
...
void solve(){ 
    ...
    REP(count,T){
        ...
        ll ans = 0LL;
        if(l<R){
            ll temp = R*(R+1)/2 + R*(-L*2+1);
            ll semp = l*(l+1)/2 + l*(-L*2+1);
            ans = temp - semp;
        }
        cout << ans << '\n';
    }
