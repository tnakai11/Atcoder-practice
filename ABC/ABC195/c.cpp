...
// 制約より愚直に調べると間に合わない
// 1 ~ 999 ... 0回 * 999
// 1,000 ~ 999,999 ... 1回 * (999999-1000+1)
// 1,000,000 ~ 999,999,999 ... 2回 *(999999999-1000000+1)
// ...
...
ll solve(){ 
  ...
  ll ans = 0LL;
  if(N<=999) return 0LL;
  if(N<=999999) return (N-1000+1);
  ans += (999999-1000+1);
  if(N<=999999999) return ans + 2*(N-1000000+1);
  ans += 2*(999999999-1000000+1);
  if(N<=999999999999) return ans + 3*(N-1000000000+1);
  ans += 3*(999999999999-1000000000+1);
  if(N<=999999999999999) return ans + 4*(N-1000000000000+1);
