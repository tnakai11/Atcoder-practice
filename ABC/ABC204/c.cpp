...
// ひとつひとつのペアを調べると O(N^2 M)になりたぶん間に合わない
// DFSで通行可能性を調べていけば O(NM + N^2)

vector<int> neighbor[2000];
int canVisit[2000][2000];
vector<int> path;
void dfs(int s, int a){
    if(canVisit[s][a]) return;
    canVisit[s][a] = 1;
...
void solve(){ 
    ...
    int ans = 0;
    REP(i,N) REP(j,N) ans += canVisit[i][j];
    cout << ans << '\n';
}
