...
// 愚直に求値すると O(N^2) となり間に合わない

// 式変形
// ans = (\sum^N_i \sum^N_j (Ai - Aj)^2 )/2
// ans*2 = 2 \sum^N_j (\sum^N_i Ai^2) - 2 (\sum^N_i \sum^N_j Ai Aj)
// ans = N \sum^N_i Ai^2 - (\sum^N_i Ai)^2
// これなら O(N) で計算可能
